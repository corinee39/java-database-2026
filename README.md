...
데이터는 단순한 컴퓨터환경의 특정 값을 의미, 정보는 데이터의 의미를 부여한 것
...
2. 설치 후 실행
    ...
    - Start Docker Destop when... 체크 후 Apply
...
2. Docker Command 사용
    ...
    - Powershell 관리자권한으로 오픈 후, 도커 실행 확인
    ...
    - 수업용 사용자 생성
        ...
        CREATE CONNECT, RESOURCE TO java;
        
        GRANT CREATE TABLE TO java;

        GRANT all privileges TO java; -- INSERT 등 추가 권한 할당
        ```

### 데이터베이스 개발툴 DBeaver 설치

1. 개발툴 종류
    - SQL*Plus - 콘솔개발 화면. 가장 기초적인 SQL 실행도구. 매우 불편
    - Oracle SQL Developer - 오라클사가 제공하는 무료 툴. 오픈소스. java 개발툴 eclipse를 커스터마이징해서 개발
    - Toad for Oracle - DB 개발툴 중 가장 강력한 SW. 상용라이선스.
...
- Database Navigator에서 DB 연결 시작
    ...
    - 입력 시 주의사항: Port 번호 확인, Database 이름변경 Oracle -> XE로, Username, Password 일치
...
- 간단 연습 : [쿼리](./day01/8.샘플쿼리.sql)
    - DB 파일은 확장자를 `.sql`
    - 하나의 명령으로서 ;으로 끝나는 문장을 쿼리(query)라고 지칭
    - 쿼리문은 대소문자 구분 없음
...
- 문법이전 데이터 타입
    - `NUMBER` - 숫자 타입, 최대 22byte
    - INTEGER - 정수 타입, 모든 데이터 기초 4byte(-21억 ~ +21억)
    - FLOAT - 실수 타입, 소수점 포함 최대 22byte
...
- 오라클 함수 : DB별로 추가 학습 필요 - [쿼리](./day02/8.오라클함수.sql)
    
    - 문자열 함수
        - `UPPER`(모두 대문자), `LOWER`(모두 소문자), `INITCAP`(첫글자만 대문자로)
        - `LENGTH`(문자열길이)
        - `SUBSTR`(문자열, 시작위치, 추출길이), 길이를 안 적으면 끝까지 추출
        - `INSTR`(문자열, 찾을 문자열, [횟수]), 문자열에서 찾을 문자열의 위치를 리턴
        - `REPLACE`(문자열, 찾는 문자열, 바꿀 문자열), 찾은 문자열을 바꿀 문자열로 변경
        - `LPAD`(문자열, 자리수, 채울 문자열), `RPAD`(문자열, 자리수, 채울 문자열), L/R 기준으로 자리수만큼 빈공간 특정문자로 채우기
        - `CONCAT`(앞쪽 문자열, 뒤쪽 문자열), 두 문자열 합치기
        - `TRIM`(공백있는 문자열), `LTRIM`(공백있는 문자열), `RTRIM`(공백있는 문자열), 문자열 앞뒤의 빈 공백 제거
...
- 날짜포맷 단어
    ...
    - DD, DDD, DY, DAY - 일 두자리(03), 62(1월1일부터 몇일째), TUE(화), TUESDAY(화요일)
...
- 오라클 함수 계속 
    - 날짜함수 - [쿼리](./day03/1.오라클함수.sql)
        ...
        - `LAST_DAY`(날짜) - 해당 날짜의 마지막일 리턴. 예) 2월 28일 3월 31일..

    - 형변환함수
        - `TO_CHAR`(날짜, '날짜포맷') - 날짜를 해당 포맷에 맞게 변경해서 표현
        - `TO_CHAR`(숫자, '날짜포맷') - 숫자를 해당 포맷에 맞게 변경해서 표현
        - `TO_NUMBER`(숫자로만 된 문자데이터, '숫자포맷') - 수로 된 문자열을 숫자로 변경
        - `TO_DATE`(날짜형식 문자데이터, '날짜포맷') - 문자데이터를 날짜데이터로 변경

    - NULL 처리함수 - [쿼리](./day03/2.NULL함수.sql)
        - NULL(데이터없음)은 일부 개수처리나 통계 불가, NULL값 처리 필요
        - NVL(NULL이 들어간 데이터, NULL 처리) - 해당 값이 NULL이면 보통 0으로 변환
        - NVL2(NULL이 들어간 데이터, NULL이 아닐때 처리, NULL일 때 처리) - NULL이 아닐 때와 NULL일 때로 나눠서 처리
...
- 그룹화 - [쿼리](./day03/5.그룹화.sql)
    ...
    - 그룹화 시 유의점
        - SELECT절에 다중행 함수 외 일반컬럼을 사용하고자 하면, 반드시 GROUP BY절에 일반컬럼이 들어가 있어야 함!
...
- 관계형 데이터베이스
    ...
    - PK - 각 행의 유일하게 식별하는 키. 여러 개의 PK를 가질 수도 있음. Primary Key
    - FK - 부모 테이블의 PK와 관계를 맺는 키. Foreign Key
...
- 조인 계속 - [쿼리](./day04/1.조인다시.sql)
    ...
    - 셀프조인 - 자체조인. 자기 테이블을 조인. 자기 테이블 내에 해당 PK와 관련있는 FK가 지정되어 있어야
        - 대부분 회사에서 조직도, 상사와 부하직원 관계 볼 때 사용
    - 외부조인 - 등가조인 반대. `Outer Join`. 조인 기준에서 일치하지 않는 데이터도 조회 나오도록 하는 조인
        - 왼쪽 외부조인 - `Left Outer Join`. 왼쪽 테이블 기준으로 오른쪽 테이블에 일치하지 않는 데이터 조회
        - 오른쪽 외부조인 - `Right Outer Join`. 오른쪽 테이블 기준으로 왼쪽 테이블에 일치하지 않는 데이터 조회
...
- DML
    ...
    - `UPDATE` - 데이터 수정 [쿼리](./day05/2.UPDATE_DELETE.sql)
        ...
        UPDATE 변경할 테이블
         ...
         WHERE 데이터 변경 대상행을 선별하기 위한 조건 -- 매우 중요!
        ```

    - `DELETE` - 데이터 삭제
        - WHERE절을 빼는 경우 정말 조심할 것!

        ```sql
        DELETE FROM 테이블
         WHERE 삭제할 대상행을 선별하는 조건  -- 매우 중요!
        ```
...
- 트랜잭션 - [쿼리](./day05/3.트랜잭션.sql)
    - 논리적으로 처리되는 쿼리들의 집합
    - 여러개 테이블에 조회, 수정, 삭제 등이 이루어지는 논리 덩어리
    - All or Nothing
...
- 세션
    - 하나의 연결로 접속해서 종료까지의 기간

- 락
    - 세션별로 트랜잭션이 문제 발생하지 않도록 데이터를 잠그는 처리
...
- DDL 명령어 - [쿼리](./day05/5.DDL.sql)
    - `CREATE` - 객체 생성. 대부분 테이블 생성 시 사용
        ...
        ```sql
        -- 문법
        CREATE [TABLE|DATABASE|VIEW|INDEX|등 객체타입][객체 생성시 필요한 문법];
    ...
    - `TRUNCATE` - 객체 내 데이터 모두 삭제. 대부분 테이블에서 진행
        ...
        ```sql
        -- DELETE FROM 테이블명 과 동일. 단, 트랜잭션이 발생하지않아 복구 불가
        -- 테이블 생성 이후 상태가 됨
        TRUNCATE TABLE 테이블명
        ```
...
- 뷰
    ...
    - 테이블의 특정 칼럼(민감한 급여, 보너스, 주민번호)을 노출하지 않을 경우
    - `주의!` 보기 위한 객체지만 한 테이블의 SELECT * 일 경우 INSERT 가능
...
- 종류
    - `NOT NULL` - 지정한 열에 NULL을 허용하지 않음. 무조건 데이터 입력
        - 데이터 중복 허용
    - `UNIQUE` - 지정한 열에 유일한 값이 되어야 함. 중복 불가
        - NULL은 중복에서 제외
    - `PRIMARY KEY` - 지정한 열에 유일한 값이면서 NULL을 허용하지 않음.
        - PK라고 하고, PK는 `UNIQUE`이면서 `NOT NULL`
        - PK를 지정하면 자동으로 UNIQUE 인덱스가 생성.
    - `FOREIGN KEY` - 다른 테이블의 PK열을 참조하여 PK열에 존재하는 값만 입력 가능
        - FK라고 하고, 설계에 따라 NOT NULL(식별관계)일 수도 있고 NULL(비식별관계)일 수도 있음.
        - 자식테이블에서 PK로 지정 가능. 일반 컬럼으로는 FK로만 지정 가능
    - `CHECK` - 설정한 조건식에 일치하는 데이터만 입력 가능
    - `DEFAULT` - 열에 데이터를 입력하지 않았을 때 기본값이 자동으로 입력됨.
...
- 사용자 생성 - [쿼리](./day07/1.사용자%20권한.sql)
...
- 예외처리 - [쿼리](./day07/6.예외처리.sql)
    - 오류 - 코드 문법 상 오류. 에러(Error)
    - 예외 - 실행 중 발생하는 오류. 예외(Exception)
    - RAISE - 예외 생성 (추후 학습 필요)
...
- 커서 - [쿼리](./day07/7.커서.sql)
    - 테이블 등에서 데이터 현재 위치를 가리키는 메모리 위치
    - DECLARE에 선언
...
- 종류
    ...
    - `트리거` - DB의 강력한 기능. 특정 상황 발생시 자동으로 연달아 수행하는 기능 구현
...
- 함수 - [쿼리](./day07/11.함수.sql)
    - 파이썬 리턴 있는 함수와 동일. 다른 SQL문에서 사용가능
...
- 패키지
    - 관련있는 함수나 프로시저 등의 객체를 그룹화시키는 개체
    - 파이썬 라이브러리와 동일
...
- 사용이유
    ...
    - SQL Plus(콘솔)에서 로그인 정보 항상 입력, 쿼리 작성시 오타 발생 농후
    - Content Assistant 등의 기능이 쿼리 작성 도와줌
...
- Schemas 내
    ...
    - Table, Views, Indexes, Sequences, Procedures, Functions, Table Triggers 위주로만 작업

- `Tables`
    - 생성된 테이블에서 Columns, Constraints, Foreign Keys, Triggers, Indexes, DDL 정도 작업
    - Tables에서 마우스 우클릭으로 컨텍스트 메뉴 중 `Create New Table`만 사용
...
- Indexes
    - 생성된 인덱스만 확인
    - Table > Create New Table. View Table에서 Indexes탭 내 Create New Index로 생성
...
- PROCEDURES
    ...
    - PROCEDURE와 FUNCTION은 컴파일 되는 객체
...
- Table Triggers
    - 생성된 트리거만 확인
    - 테이블의 Triggers 탭에서 Create New Trigger로 생성. Declaration에서 작성
    - 일반적인 SQL 에디터에서 작성
...
- 데이터모델링
    - 현실세계의 데이터를 DB 내에 옮기기 위해 데이터베이스를 설계
...
- 논리 데이터 모델링
    - 개념 데이터 모델링을 바탕으로 속성, 키, 관계를 명확히 정의
    - 데이터 중복을 최소화하기위한 **정규화** 수행
...
- 종류
    ...
    - DB설계자와 업무담당자(병원관계자)와 협의
        1. 의사 관리, 간호사 관리, 환자 관리, 진료과 관리, 입원 처리, 진료 처리
            - `의사`, `간호사`, `환자`, `진료과` -> 명사
            - `입원`, `진료` -> 동사
        2. 의사는 고유의사번호, 이름, 진료과목, 주소, 연락처, 주민번호, 직급, 과번호, 의사약력, ...
        3. 간호사는 고유간호사번호, 이름, 진료과목, 주소, 연락처, 주민번호, 직급, 과번호, ...
        4. 환자는 고유환자번호, 이름, 주민번호, 연락처, 주소, 키, 몸무게, 혈액형, 성별, 진료과, ...
        5. 입원은 입원일, 퇴원일, 병실 정보
        6. 진료과는 고유의 과번호, 과이름, 전화번호, 팩스번호, ...
        7. 의사는 여러명의 환자를 진찰하고, 환자도 여러 명의 의사에게 진료받을 수 있음
    ...
    - DB설계자의 개념 설계
        ...
        - 관계 추출
            ...
            - ~~`퇴원` - `환자`가 `병실`에서 퇴원~~
            - 다대다(n:m) 관계는 만들지 않음. 물리 DB에서 이를 구현할 수도 없음. `추후학습요망`
    ...
    - 도메인
        - 특정 컬럼에 들어가는 값의 범위
        - 초등학교 1~6학년, 대학교 1~4학년

    - DB 설계는 일회성이 아니고, 중간에 계속 변경 가능
...
- ERDCloud에서 식별관계 SQL 쿼리를 제대로 생성못함
